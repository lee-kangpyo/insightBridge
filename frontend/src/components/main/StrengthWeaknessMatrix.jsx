...
export default function StrengthWeaknessMatrix({ matrix }) {
  ...
  const hasPoints = Boolean(matrix?.points?.length);
    ...
    <div className="lg:col-span-2 bg-surface-container-lowest p-8 rounded-xl shadow-[0_8px_32px_rgba(24,28,30,0.04)]">
      <div className="flex justify-between items-center mb-8">
        <h3 className="text-lg font-bold text-primary flex items-center gap-2">
          ...
          <span>{matrix?.title || "주요 지표 강점/약점 매트릭스"}</span>
        </h3>
        {hasMatrix ? (
          <div className="flex gap-4 text-[10px] font-bold text-outline">
            <div className="flex items-center gap-1">
              <span className="w-2 h-2 rounded-full bg-secondary"></span> 지역
              대비
            </div>
            <div className="flex items-center gap-1">
      ...
      <div className="relative h-[400px] border-l border-b border-outline-variant/30 flex items-center justify-center">
        {!hasMatrix || !hasPoints ? (
          <EmptyState
            title="미공시"
            description="강점/약점 매트릭스 데이터가 미공시입니다."
            minHeight={360}
            icon="scatter_plot"
          />
        ) : null}
        ...
        <div className="absolute inset-0 grid grid-cols-2 grid-rows-2">
          <div className="bg-tertiary-fixed/10 flex items-start p-4">
            <span className="text-[10px] font-black text-tertiary/40">
              집중 홍보 구간 (지역/전국 우위)
            </span>
          </div>
          ...
          <div className="bg-error-container/5 flex items-end p-4">
            <span className="text-[10px] font-black text-error/40">
              전국 열세 개선
            </span>
          </div>
        ...
        {matrix?.points?.map((point) => (
          ...
          >
            <div
              ...
              style={
                point.colorHex ? { backgroundColor: point.colorHex } : undefined
              }
            ></div>
