  ...
  return (
    <div className="fixed inset-0 z-50">
      <button
        ...
        onClick={onClose}
      />
      <div className="relative mx-auto mt-[8vh] w-[min(900px,calc(100%-2rem))] h-[min(640px,calc(92vh-2rem))] rounded-2xl border border-outline-variant/40 bg-surface-container-lowest shadow-[0_24px_80px_rgba(0,0,0,0.25)] overflow-hidden flex flex-col">
        <div className="h-1.5 w-full bg-gradient-to-r from-primary to-primary-container shrink-0" />
        <div className="p-5 sm:p-6 flex items-start justify-between gap-4 shrink-0">
          <div>
            <div className="text-xs font-medium tracking-wider text-on-surface-variant uppercase">
              Slot Screen
            </div>
            ...
            <p className="mt-1 text-sm text-on-surface-variant leading-relaxed">
              연동할 슬롯 화면을 선택하면 메뉴가 자동 생성됩니다. 이미 다른 메뉴에서 사용 중인 화면도 선택할 수 있습니다.
            </p>
          </div>
          <button
            type="button"
        ...
        {error && (
          <div className="mx-5 sm:mx-6 mb-3 p-3 bg-error/10 border border-error rounded-lg text-error text-sm shrink-0">
            {error}
          </div>
        )}

        <div className="flex-1 min-h-0 flex flex-col sm:flex-row gap-0 mx-5 sm:mx-6 mb-4 border border-outline-variant/40 rounded-xl overflow-hidden">
          {/* Left: Screen List */}
        ...
        <div className="flex flex-col sm:flex-row gap-3 justify-end px-5 sm:px-6 pb-5 sm:pb-6 shrink-0">
          <button
            type="button"
            className="h-11 px-4 rounded-lg border border-outline-variant/40 bg-surface-container-lowest text-on-surface hover:bg-surface-container-high transition-colors font-medium"
            onClick={onClose}
